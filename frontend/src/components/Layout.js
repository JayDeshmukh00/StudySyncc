...
export const Header = ({ onToggleTheme, currentTheme, onHomeClick, onLogout, user, onSetView }) => {
    // ... Header is unchanged
    ...
    const navItems = [
        ...
        { label: 'Aura Reader', view: 'auraReader' },
        { label: 'Calendar', view: 'calendar' },
        { label: 'Flashcards', view: 'flashcards' },
        { label: 'Study Sessions', view: 'studysessions' },
        { label: 'Mind Map', view: 'mindmap' },
    ];
            ...
            <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                <div className="flex items-center space-x-3 cursor-pointer" onClick={onHomeClick}>
                   <img src="/sslogo.png" alt="Logo" className="w-10 h-10 rounded-full" />
                    ...
                    <h1 className="text-2xl font-medium text-gray-700 dark:text-gray-300 dynamic-heading">StudySync</h1>
                </div>
                {user && (
                ...
                {user && (
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-cyan-600 to-blue-800 dark:from-gray-800 dark:to-black text-white rounded-full font-bold text-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 dark:focus:ring-offset-gray-900 focus:ring-cyan-600 hover:shadow-lg hover:shadow-cyan-600/50 dark:hover:shadow-black/50 transition-all duration-300 hover:scale-110">
                                {getInitials(user.name)}
                            </button>
                            {isDropdownOpen && (
                                <div ref={dropdownRef} className="absolute right-0 mt-2 w-56 bg-white dark:bg-black rounded-md shadow-lg py-1 ring-1 ring-black ring-opacity-5 z-50 animate-fade-in">
...
useEffect(() => {
    ...
    if (!isOpen) {
        return (
            <button onClick={() => setIsOpen(true)} className="fixed bottom-5 right-5 z-50 w-16 h-16 bg-gradient-to-br from-cyan-800 to-blue-900 dark:from-black dark:to-gray-900 rounded-full text-white flex items-center justify-center shadow-lg hover:shadow-cyan-800/50 dark:hover:shadow-gray-900/50 transition-all duration-300 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-cyan-600 focus:ring-offset-2 dark:focus:ring-offset-gray-900" aria-label="Open Chatbot">
